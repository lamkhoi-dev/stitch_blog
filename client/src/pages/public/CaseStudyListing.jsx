...
const CaseStudyListing = () => {
  const { articles: caseStudies, isLoading, error } = useArticles({ category: 'case-study', limit: 12 });
      ...
      <section className="mb-20 grid grid-cols-1 md:grid-cols-12 gap-8 items-end">
        <div className="md:col-span-4 group">
          ...
          <div className="relative flex items-center">
            ...
            <input className="w-full pl-12 pr-4 py-4 bg-surface-container-low border-b border-outline-variant focus:border-primary focus:ring-0 transition-all text-sm font-medium placeholder:text-outline/60" placeholder="Nhập từ khóa hoặc mã dự án..." type="text" />
          </div>
        </div>
        <div className="md:col-span-8 overflow-x-auto pb-2">
          ...
          <div className="flex gap-3">
            <button className="px-5 py-2 bg-primary text-surface-bright text-xs font-semibold rounded-sm whitespace-nowrap">Tất cả bài viết</button>
            {['FMCG', 'Retail & E-commerce', 'Manufacturing', 'Pharmaceuticals', 'Energy & Oil'].map(tag => (
              <button key={tag} className="px-5 py-2 bg-surface-container-high text-primary text-xs font-semibold rounded-sm hover:bg-surface-container-highest transition-colors whitespace-nowrap">{tag}</button>
            ))}
          </div>
        </div>
      </section>
